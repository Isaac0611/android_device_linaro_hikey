 ...
 *
 * It is not clear if the USB audio device is able to control volumes, or if this is
 * something that we have to handle manually for individual channel FADE function.
 *
...
// TODO example function for webrtc processing.
...
//    frame._frequencyInHz = 8000;
//    frame._payloadDataLengthInSamples = 8000/100; // Math for 10ms frames
...
//    const size_t frameLength = frame._payloadDataLengthInSamples*1;
...
static int adev_open(const hw_module_t* module, const char* name, hw_device_t** device)
{
    ...
    struct audio_device *adev = (audio_device *)calloc(1, sizeof(struct audio_device));
    if (!adev)
        return -ENOMEM;
